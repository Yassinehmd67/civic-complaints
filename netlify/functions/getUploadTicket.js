...
function ipFromHeaders(h = {}) {
  // Netlify يمرر X-Forwarded-For، وقد توجد بدائل
  const raw =
    h["x-forwarded-for"] ||
    h["X-Forwarded-For"] ||
...
export async function handler(event) {
  ...
  const {
    SUPABASE_URL,
    SUPABASE_SERVICE_ROLE,  // 🔒 مفتاح خدمة Supabase
    SUPABASE_BUCKET,        // 🪣 اسم الباكت (مثلاً proofs)
    HCAPTCHA_SECRET,        // 🔑 سر hCaptcha (من لوحة hCaptcha)
  } = process.env;
  ...
  if (Object.values(missing).some((ok) => !ok)) {
    return cors({
      statusCode: 500,
      body: JSON.stringify({
  ...
  let captchaToken = String(body.captchaToken || "").trim();
  ...
  try {
    ...
    if (!verifyJson.success) {
      // ملاحظة: من أشهر الأكواد already-seen-response عند إعادة استخدام التوكن
      return cors({
        statusCode: 400,
        body: JSON.stringify({
  ...
  // إنشاء تذكرة رفع موقعة من Supabase
  try {
    ...
    if (!ext) {
      return cors({
        statusCode: 415,
        body: JSON.stringify({ error: "Unsupported mime extension" }),
      });
    }
...
// نجاح: نعيد المسار والتوكن للواجهة
    return cors({
      statusCode: 200,
